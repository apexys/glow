...
impl Context {
    fn is_extension_loaded(ext_pointer: &core::sync::atomic::AtomicUsize) -> bool{
        ext_pointer.load(std::sync::atomic::Ordering::Acquire) != 0
    }

    pub unsafe fn from_loader_function_cstr<F>(loader_function: F) -> Self
    where
        F: Fn(&CStr) -> *const std::os::raw::c_void,
    {
        let loader_closure = Box::new(move |p: *const u8| {
            let c_str = std::ffi::CStr::from_ptr(p as *const i8);
            let x = loader_function(c_str); 
            x as *const std::os::raw::c_void
        });

        if let Err(e) = native_gl::load_gl_functions(&loader_closure){
            panic!("Failed to load the following functions {e:?}");
        }

        // Retrieve and parse `GL_VERSION`
        let raw_string = native_gl::glGetString(VERSION);
...
impl HasContext for Context {
    ...
    unsafe fn create_shader(&self, shader_type: u32) -> Result<Self::Shader, String> {
        Ok(NativeShader(non_zero_gl_name(
            native_gl::glCreateShader(shader_type as u32),
        )))
    }
    ...
    unsafe fn get_shader_completion_status(&self, shader: Self::Shader) -> bool {
        let mut status = 0;
        native_gl::glGetShaderiv(shader.0.get(), COMPLETION_STATUS, &mut status);
        1 == status
    }
    ...
    unsafe fn get_tex_sub_image(
        ...
        native_gl::glGetTextureSubImage(
            target,
            level,
            x_offset,
            y_offset,
            ...
            ty,
            match pixels{
                PixelPackData::BufferOffset(_) => 0,
                PixelPackData::Slice(ref data) => data.len() as u32,
            }, 
            match pixels {
                PixelPackData::BufferOffset(offset) => offset as *mut std::ffi::c_void,
                PixelPackData::Slice(data) => data.as_mut_ptr() as *mut std::ffi::c_void,
    ...
    unsafe fn create_program(&self) -> Result<Self::Program, String> {
        Ok(NativeProgram(non_zero_gl_name(native_gl::glCreateProgram())))
    }
    ...
    unsafe fn get_program_completion_status(&self, program: Self::Program) -> bool {
        let mut status = 0;
        native_gl::glGetProgramiv(program.0.get(), COMPLETION_STATUS, &mut status);
        1 == status
    }
    ...
    unsafe fn get_program_resource_i32(
        ...
        let mut length = 0u32;
        native_gl::glGetProgramResourceiv(
            program.0.get(),
            interface,
            index,
    ...
    unsafe fn get_active_uniform(
        ...
        let uniform_max_size = uniform_max_size as u32;
    ...
    unsafe fn copy_tex_image_2d(
        ...
        native_gl::glCopyTexImage2D(target, level, internal_format, x, y, width as u32, height as u32, border);
    }
    ...
    unsafe fn copy_tex_sub_image_2d(
        ...
        native_gl::glCopyTexSubImage2D(target, level, x_offset, y_offset, x, y, width as u32, height as u32);
    }
    ...
    unsafe fn draw_arrays(&self, mode: u32, first: i32, count: i32) {
        native_gl::glDrawArrays(mode as u32, first, count as u32);
    }

    unsafe fn draw_arrays_instanced(&self, mode: u32, first: i32, count: i32, instance_count: i32) {
        native_gl::glDrawArraysInstanced(mode as u32, first, count as u32, instance_count as u32);
    }
    ...
    unsafe fn draw_buffers(&self, buffers: &[u32]) {
        native_gl::glDrawBuffers(buffers.len() as u32, buffers.as_ptr());
    }
    ...
    unsafe fn renderbuffer_storage(
        ...
        native_gl::glRenderbufferStorage(target, internal_format, width as u32, height as u32);
    }
    ...
    unsafe fn renderbuffer_storage_multisample(
        ...
        native_gl::glRenderbufferStorageMultisample(target, samples as u32, internal_format, width as u32, height as u32);
    }
    ...
    unsafe fn tex_storage_1d(&self, target: u32, levels: i32, internal_format: u32, width: i32) {
        native_gl::glTexStorage1D(target, levels as u32, internal_format, width as u32);
    }
    ...
    unsafe fn tex_storage_2d(
        ...
        native_gl::glTexStorage2D(target, levels as u32, internal_format, width as u32, height as u32);
    }
    ...
    unsafe fn tex_storage_3d(
        ...
        native_gl::glTexStorage3D(target, levels as u32, internal_format, width as u32, height as u32, depth as u32);
    }
    ...
    unsafe fn uniform_1_i32_slice(&self, location: Option<&Self::UniformLocation>, v: &[i32]) {
        if let Some(loc) = location {
            native_gl::glUniform1iv(loc.0 as i32, v.len() as u32, v.as_ptr());
        }
    }

    unsafe fn uniform_2_i32_slice(&self, location: Option<&Self::UniformLocation>, v: &[i32]) {
        if let Some(loc) = location {
            native_gl::glUniform2iv(loc.0 as i32, v.len() as u32 / 2, v.as_ptr());
        }
    }

    unsafe fn uniform_3_i32_slice(&self, location: Option<&Self::UniformLocation>, v: &[i32]) {
        if let Some(loc) = location {
            native_gl::glUniform3iv(loc.0 as i32, v.len() as u32 / 3, v.as_ptr());
        }
    }

    unsafe fn uniform_4_i32_slice(&self, location: Option<&Self::UniformLocation>, v: &[i32]) {
        if let Some(loc) = location {
            native_gl::glUniform4iv(loc.0 as i32, v.len() as u32 / 4, v.as_ptr());
        }
    }
    ...
    unsafe fn uniform_1_u32_slice(&self, location: Option<&Self::UniformLocation>, v: &[u32]) {
        if let Some(loc) = location {
            native_gl::glUniform1uiv(loc.0 as i32, v.len() as u32, v.as_ptr());
        }
    }

    unsafe fn uniform_2_u32_slice(&self, location: Option<&Self::UniformLocation>, v: &[u32]) {
        if let Some(loc) = location {
            native_gl::glUniform2uiv(loc.0 as i32, v.len() as u32 / 2, v.as_ptr());
        }
    }

    unsafe fn uniform_3_u32_slice(&self, location: Option<&Self::UniformLocation>, v: &[u32]) {
        if let Some(loc) = location {
            native_gl::glUniform3uiv(loc.0 as i32, v.len() as u32 / 3, v.as_ptr());
        }
    }

    unsafe fn uniform_4_u32_slice(&self, location: Option<&Self::UniformLocation>, v: &[u32]) {
        if let Some(loc) = location {
            native_gl::glUniform4uiv(loc.0 as i32, v.len() as u32 / 4, v.as_ptr());
        }
    }
    ...
    unsafe fn uniform_1_f32_slice(&self, location: Option<&Self::UniformLocation>, v: &[f32]) {
        if let Some(loc) = location {
            native_gl::glUniform1fv(loc.0 as i32, v.len() as u32, v.as_ptr());
        }
    }

    unsafe fn uniform_2_f32_slice(&self, location: Option<&Self::UniformLocation>, v: &[f32]) {
        if let Some(loc) = location {
            native_gl::glUniform2fv(loc.0 as i32, v.len() as u32 / 2, v.as_ptr());
        }
    }

    unsafe fn uniform_3_f32_slice(&self, location: Option<&Self::UniformLocation>, v: &[f32]) {
        if let Some(loc) = location {
            native_gl::glUniform3fv(loc.0 as i32, v.len() as u32 / 3, v.as_ptr());
        }
    }

    unsafe fn uniform_4_f32_slice(&self, location: Option<&Self::UniformLocation>, v: &[f32]) {
        if let Some(loc) = location {
            native_gl::glUniform4fv(loc.0 as i32, v.len() as u32 / 4, v.as_ptr());
        }
    }
    ...
    unsafe fn color_mask(&self, red: bool, green: bool, blue: bool, alpha: bool) {
        native_gl::glColorMask(red as u32, green as u32, blue as u32, alpha as u32);
    }
    ...
    unsafe fn color_mask_draw_buffer(
        ...
        native_gl::glColorMaski(draw_buffer, red as u32, green as u32, blue as u32, alpha as u32);
    }

    unsafe fn depth_mask(&self, value: bool) {
        native_gl::glDepthMask(value as u32);
    }
    ...
    unsafe fn invalidate_framebuffer(&self, target: u32, attachments: &[u32]) {
        native_gl::glInvalidateFramebuffer(target, attachments.len() as u32, attachments.as_ptr());
    }
    ...
    unsafe fn fence_sync(&self, condition: u32, flags: u32) -> Result<Self::Fence, String> {
        Ok(NativeFence(native_gl::glFenceSync(condition as u32, flags)))
    }
    ...
    unsafe fn depth_range_f64_slice(&self, first: u32, count: i32, values: &[[f64; 2]]) {
        native_gl::glDepthRangeArrayv(first, count as u32, values.as_ptr() as *const f64);
    }

    unsafe fn scissor(&self, x: i32, y: i32, width: i32, height: i32) {
        native_gl::glScissor(x, y, width as u32, height as u32);
    }

    unsafe fn scissor_slice(&self, first: u32, count: i32, scissors: &[[i32; 4]]) {
        native_gl::glScissorArrayv(first, count as u32, scissors.as_ptr() as *const i32);
    }
    ...
    unsafe fn vertex_attrib_format_f32(
        ...
        native_gl::glVertexAttribFormat(index, size, data_type, normalized as u32, relative_offset);
    }
    ...
    unsafe fn viewport(&self, x: i32, y: i32, width: i32, height: i32) {
        native_gl::glViewport(x, y, width as u32, height as u32);
    }

    unsafe fn viewport_f32_slice(&self, first: u32, count: i32, values: &[[f32; 4]]) {
        native_gl::glViewportArrayv(first, count as u32, values.as_ptr() as *const f32);
    }
        ...
        F: FnMut(u32, u32, u32, u32, &str) + 'static,
    {
        match self.debug_callback {
            ...
            None => {
                ...
                if Self::is_extension_loaded(&native_gl::glDebugMessageCallbackARB_p){
                    native_gl::glDebugMessageCallback(Some(raw_debug_message_callback), raw_ptr);
                } else {
                    // Fallback to extension
                    native_gl::glDebugMessageCallbackKHR(Some(raw_debug_message_callback), raw_ptr);
                }
    ...
    unsafe fn get_uniform_block_index(&self, program: Self::Program, name: &str) -> Option<u32> {
        let name = CString::new(name).unwrap();
        let index = native_gl::glGetUniformBlockIndex(program.0.get(), name.as_ptr() as *const u8);
    ...
    unsafe fn get_shader_storage_block_index(
        ...
        let name = CString::new(name).unwrap();
        let index =
            native_gl::glGetProgramResourceIndex(program.0.get(), SHADER_STORAGE_BLOCK, name.as_ptr() as *const u8);
        if index == INVALID_INDEX {
            None
        } else {
            Some(index)
    ...
    unsafe fn query_counter(&self, query: Self::Query, target: u32) {
        native_gl::glQueryCounter(query.0.get(), target);
    }
    ...
    unsafe fn get_query_parameter_u64_with_offset(
        ...
        native_gl::glGetQueryObjectui64v(query.0.get(), parameter, offset as *mut _);
    }
    ...
    unsafe fn bind_transform_feedback(
        ...
        native_gl::glBindTransformFeedback(target, transform_feedback.map(|tf| tf.0.get()).unwrap_or(0));
    }
    ...
    unsafe fn get_active_uniform_block_parameter_i32(
        ...
        let mut value = 0;
        native_gl::glGetActiveUniformBlockiv(program.0.get(), uniform_block_index, parameter, &mut value);
        value
    }
    ...
    unsafe fn get_active_uniform_block_name(
        ...
        let len = if native_gl::glGetError() == crate::NO_ERROR && len > 0 {
            len as usize
        } else {
            256
        };
    ...
    unsafe fn max_shader_compiler_threads(&self, count: u32) {
        if Self::is_extension_loaded(&native_gl::glMaxShaderCompilerThreadsARB_p){
            native_gl::glMaxShaderCompilerThreadsKHR(count);
        } else {
            native_gl::glMaxShaderCompilerThreadsARB(count);
        }
    }
}
